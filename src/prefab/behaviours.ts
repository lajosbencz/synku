...
export const withDefaultResources = (resources: { requests: { cpu: string, memory: string }, limits: { cpu: string, memory: string } }): IBehaviour => {
  return (component: IComponent) => {
    for (const manifest of component.findManifests(k8s.apps.v1.Deployment)) {
      manifest.spec?.template?.spec?.containers?.map(container => container.resources = {
        ...container.resources,
        requests: {
